...
"""
The shapefile provided by Frederick County, MD was generated by ArcGIS. The EPSG is 2248 for Maryland.
In order to overlay the Maryland on world maps, it requires to convert those coordinates under EPSG 4326.
...
class Shape2Json:
    def __init__(self, fname, output1, output2, school_param, school_list, addresses = None, coordinates = None):
        self.fname = fname
    ...
    def convert_json(self): # schoolPara refers to 'SCHOOL_1','SCHOOL' in different shapefiles
        ...
        for record in reader.shapeRecords():
            ...
            if attributes[self.school_param] in self.school_list:
                ...
                features.append(dict(type = 'Feature', geometry = geo_records, properties = attributes))
            else:
                continue

        json_file = open(self.output1, 'w')
        json_file.write(dumps({'type': 'FeatureCollection', 'features': features}, indent = 2) + '\n')
        json_file.close()
            ...
            for feature in features_old:
                ...
                if type == 'Polygon':
                    for coordinate in records:
                        coordinate_new = transform(in_proj, out_proj, coordinate[0], coordinate[1])
                        coordinates.append([coordinate_new[0], coordinate_new[1]])
                ...
                features_new.append(dict(type = 'Feature', geometry = geo_new, properties = attributes))

        json_file = open(self.output2, 'w')
        json_file.write(dumps({'type': 'FeatureCollection', 'features': featuresNew}, indent = 2) + '\n')
        json_file.close()
