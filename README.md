...
It should be noted that the algorithm of the sentiment analysis is optimized for this project and needs tweaks, such as incorporating with Natural Language Toolkit (`nltk`), to apply to other similar projects. Additionally, the json files for visualization may need further cleaned up in order to plot a complete polygon for each school district. However, because of the constrained timing, the results of current project can still be used as good initial assessments for understanding the needs of parents after the Engagement Session on June, 2019. Future revision needs to address those issues mentioned above.
